...
const getAssignedFromGroup = (courseId: number): string | null => {
  ...
  for (const [assignedId] of courseAssignments.value) {
    if (assignedId !== courseId && getCourseGroup(assignedId) === courseGroup && getDefaultSemester(assignedId) === defaultSem) {
      const course = coursesData.value?.courses.find(c => c.course_id === assignedId);
...
const quickAssign = (course: AvailableCourse) => {
  if (selectedSemester.value !== null && !isTransferred(course.course_id) && !isAssigned(course.course_id)) {
...
const clearAllCourses = () => {
  if (confirm("Are you sure you want to unassign all courses? This will clear all assignments.")) {
...
<template>
  <div class="p-4 w-full h-[calc(100vh-64px)] flex flex-col overflow-hidden">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
          ...
          <div class="flex items-center gap-2">
            <label class="label cursor-pointer gap-2">
              ...
              <input type="checkbox" v-model="showProgramStructure" class="toggle toggle-sm toggle-primary" />
            </label>
            <button
    ...
    <template v-else-if="planData && coursesData">
      <!-- Summary Bar -->
      <div class="flex flex-wrap items-center gap-4 mb-4 p-3 bg-base-200 rounded-lg flex-shrink-0">
        <div class="flex items-center gap-2">
          <span class="text-sm text-base-content/60">To Schedule:</span>
          <span class="font-semibold text-primary">{{ totalAssignedCredits }} / {{ planData.summary.planned_credits }} cr</span>
        </div>
        <div class="flex items-center gap-2">
        ...
        <div class="flex items-center gap-2">
          ...
          <span class="font-semibold" :class="plannedCourses.filter(c => !isAssigned(c.course_id)).length > 0 ? 'text-warning' : 'text-success'">
            {{ plannedCourses.filter(c => !isAssigned(c.course_id)).length }} courses
          </span>
        </div>
      </div>
      ...
      <div class="grid gap-4 flex-1 min-h-0" :class="showProgramStructure ? 'lg:grid-cols-2' : ''">
        <!-- Left: Current Semester Schedule -->
        <div class="flex flex-col min-h-0 bg-base-100 rounded-lg border border-base-300 overflow-hidden">
          <!-- Fixed Header -->
          ...
          <div class="flex-1 overflow-y-auto p-3 space-y-3">
            <!-- Assigned Courses -->
            <div>
              <h3 class="text-xs font-medium text-base-content/60 mb-2 uppercase tracking-wide">Assigned to this Semester</h3>
              <div class="space-y-1">
                <div
                ...
                >
                  <div class="flex items-center gap-2 min-w-0">
                    ...
                    <span class="text-sm truncate">{{ course.course_name }}</span>
                    <span class="badge badge-xs shrink-0">{{ course.credit_hour }}cr</span>
                  </div>
                  <button class="btn btn-ghost btn-xs text-error shrink-0" @click="removeCourse(course.course_id)">✕</button>
                </div>
                <div v-if="plannedCourses.filter(c => courseAssignments.get(c.course_id) === selectedSemester).length === 0" class="text-center py-4 text-base-content/40 text-sm">
            ...
            <!-- Unassigned Courses -->
            <div>
              <h3 class="text-xs font-medium text-base-content/60 mb-2 uppercase tracking-wide">Unassigned Courses (Click to Add)</h3>
              <div class="space-y-1">
                <div
                  ...
                  :class="isCourseDisabled(course.course_id) 
                    ? 'border-base-300 bg-base-200/30 opacity-50 cursor-not-allowed' 
                    : 'border-base-300 hover:bg-base-200 cursor-pointer'"
                  @click="!isCourseDisabled(course.course_id) && assignCourse(course.course_id, selectedSemester!)"
                >
                  <div class="flex items-center gap-2 min-w-0">
                    <span class="font-mono text-xs shrink-0">{{ course.course_code }}</span>
                    <span class="text-sm truncate">{{ course.course_name }}</span>
                  ...
                  <span v-else class="text-base-content/30 text-sm shrink-0">—</span>
                </div>
                <div v-if="plannedCourses.filter(c => !courseAssignments.has(c.course_id)).length === 0" class="text-center py-4 text-success text-sm">
        ...
        <div v-if="showProgramStructure" class="flex flex-col min-h-0 bg-base-100 rounded-lg border border-base-300 overflow-hidden">
          <!-- Fixed Header -->
          <div class="p-3 border-b border-base-300 flex-shrink-0 bg-base-100">
            <div class="flex items-center justify-between">
              <h2 class="font-semibold">📚 Program Structure</h2>
              <div class="flex items-center gap-2 text-xs">
                <span class="flex items-center gap-1"><span class="w-2 h-2 rounded bg-success"></span> Transferred</span>
                <span class="flex items-center gap-1"><span class="w-2 h-2 rounded bg-primary"></span> Assigned</span>
              </div>
            </div>
            <p v-if="selectedSemester" class="text-xs text-base-content/60 mt-1">
              💡 Click on unassigned courses to add to {{ formatSemester(selectedSemester) }}
            </p>
          </div>

          <!-- Scrollable Content -->
          <div class="flex-1 overflow-y-auto p-3">
            <div v-for="[sem, courses] in Array.from(programStructure).sort((a, b) => a[0] - b[0])" :key="sem" class="mb-4 last:mb-0">
              <div class="flex items-center justify-between mb-2 sticky top-0 bg-base-100 py-1 -mt-1">
                <h4 class="font-medium text-sm flex items-center gap-2">
                  <span :class="sem === selectedSemester ? 'text-primary' : ''">{{ formatSemester(sem) }}</span>
                  <span v-if="sem === selectedSemester" class="badge badge-primary badge-xs">Current</span>
                </h4>
              ...
              <div class="space-y-1">
                <div
                  ...
                  :class="{
                    'bg-success/20 border border-success/30': isTransferred(course.course_id),
                    'bg-primary/10 border border-primary/20': isAssigned(course.course_id) && !isTransferred(course.course_id),
                    'bg-base-200/30 opacity-50 cursor-not-allowed': !isAssigned(course.course_id) && !isTransferred(course.course_id) && isCourseDisabled(course.course_id),
                    'bg-base-200/50 hover:bg-base-200 cursor-pointer': !isAssigned(course.course_id) && !isTransferred(course.course_id) && !isCourseDisabled(course.course_id)
                  }"
                  @click="!isCourseDisabled(course.course_id) && quickAssign(course)"
                >
                  <div class="flex items-center gap-2 min-w-0">
                    <span class="font-mono text-xs shrink-0">{{ course.course_code }}</span>
                  ...
                  <div class="flex items-center gap-1 shrink-0">
                    <span class="badge badge-xs">{{ course.credit_hour }}cr</span>
                    <span v-if="isTransferred(course.course_id)" class="badge badge-xs badge-success">T</span>
                    <span v-else-if="isAssigned(course.course_id)" class="badge badge-xs badge-primary">
                      S{{ getAssignedSemester(course.course_id) }}
                    </span>
                    <span v-else-if="isCourseDisabled(course.course_id)" class="badge badge-xs badge-warning">{{ getAssignedFromGroup(course.course_id) }}</span>
      ...
      <div v-if="transferredCourses.length > 0" class="collapse collapse-arrow bg-success/5 border border-success/20 rounded-lg mt-4 flex-shrink-0">
        <input type="checkbox" />
