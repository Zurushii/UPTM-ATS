    ...
    if (limits) {
      ...
      if (currentCredits + course.credit_hour > limits.max) {
        const reason = coursesData.value?.on_probation
          ? `Your CGPA is below 2.5. You can only take up to ${limits.max} credit hours per semester.`
    ...
    <template v-else-if="planData && coursesData">
      ...
      >
        ...
        <div>
          ...
          <div class="text-sm">
            Your CGPA is below 2.5. You are restricted to the minimum credit
            hours only ({{ coursesData.credit_limits.long_max }} cr for long
