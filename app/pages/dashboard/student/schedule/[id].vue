...
const failedCourseIds = computed(() => {
  ...
  // Exclude from failed if also passed (retake succeeded)
...
const retakeCourses = computed(() => {
  ...
  return coursesData.value.retake_courses.filter((c) => {
    ...
    // Show in pool if not assigned, or still in original failed semester
...
const applyConfig = () => {
  if (!coursesData.value) return;
  // Reset target_credits if type changed
  ...
  // Sync extraSemesters for any semesters beyond what data originally had
...
const isPrereqSatisfied = (
  ...
  // Transferred courses always satisfy prerequisites
  ...
  <div class="p-4 w-full h-[calc(100vh-64px)] flex flex-col overflow-hidden relative">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p v-if="planData" class="text-sm text-base-content/60">
              My Academic Plan — <span class="capitalize">{{ planData.plan.status }}</span>
            </p>
          </div>
          <div class="flex items-center gap-2">
            ...
            <button class="btn btn-outline btn-sm gap-2" @click="isPreviewAllOpen = true">
            ...
            <button class="btn btn-primary btn-sm" :disabled="saveLoading" @click="saveChanges">
    ...
    <template v-else-if="planData && coursesData">
      ...
      <div class="flex gap-4 flex-1 min-h-0">
        <!-- Left: Course Pool -->
        <div
          class="flex-1 flex flex-col min-h-0 bg-base-100 rounded-lg border overflow-hidden transition-all duration-200"
            ...
            <div v-for="[sem, courses] in Array.from(filteredPoolCourses).sort((a, b) => a[0] - b[0])" :key="'pool-' + sem">
              ...
              <div class="space-y-1">
                <div
                  ...
                  class="pool-card flex items-center gap-2 p-2 rounded border text-sm transition-all duration-150"
            ...
            <div v-if="filteredRetakeCourses.length > 0">
              ...
              <div class="space-y-1">
                <div
                  ...
                  class="pool-card flex items-center gap-2 p-2 rounded border text-sm border-error/30 bg-error/5 hover:bg-error/10 cursor-grab active:cursor-grabbing transition-all duration-150"
                  draggable="true"
                  @dragstart="onDragStart($event, course.course_id, 'pool')"
                  @dragend="onDragEnd"
                >
        ...
        <div class="flex-1 flex flex-col min-h-0 bg-base-100 rounded-lg border border-base-300 overflow-hidden">
          ...
          <div class="flex items-center gap-2 px-3 py-2 border-b border-base-300 flex-shrink-0 bg-base-100">
            ...
            <div class="dropdown">
              ...
              <ul tabindex="0" class="dropdown-content menu bg-base-100 rounded-box shadow-lg border border-base-300 w-64 p-1.5 z-50 mt-1">
          ...
          <div
            class="flex-1 overflow-y-auto p-4 transition-all duration-200"
    ...
    <Transition name="toast">
      <div v-if="toastMessage" class="fixed bottom-6 right-6 z-50 alert shadow-lg max-w-sm"
        ...
        <div class="flex items-start justify-between mb-1 flex-shrink-0">
          <div>
            ...
            <p v-if="planData" class="text-sm text-base-content/60 mt-0.5">
              My Academic Plan &mdash; Read-only preview
            </p>
          </div>
