...
<template>
  ...
  >
    <!-- Menu -->
    <ul
      class="menu bg-base-100 w-full rounded-box shadow-sm gap-2 transition-all duration-300"
    >
      ...
      <template v-if="user?.role === 'HOP'">
        <li>
          <NuxtLink
            ...
            @click="handleClick"
            :class="[
              { active: isActive('/dashboard/hop/students') },
              expanded ? '' : 'justify-center',
            ]"
            class="flex items-center"
        ...
        <li>
          <NuxtLink
            to="/dashboard/hop/course-search"
            @click="handleClick"
            :class="[
              { active: isActive('/dashboard/hop/course-search') },
              expanded ? '' : 'justify-center',
            ]"
            class="flex items-center"
            :title="!expanded ? 'Course Search' : ''"
          >
            <span class="text-xl">🔍</span>
            <span v-if="expanded" class="ml-3 truncate">Course Search</span>
          </NuxtLink>
        </li>
      </template>
      ...
      <!-- Student Section -->
      <li
        v-if="user?.role === 'STUDENT' && expanded"
        class="menu-title transition-opacity duration-300"
      >
        Academic
      </li>
      <li
        v-else-if="user?.role === 'STUDENT' && !expanded"
        class="menu-title text-center px-0"
      >
        A
      </li>
      ...
      <template v-if="user?.role === 'STUDENT'">
        <li>
          <NuxtLink
            ...
            @click="handleClick"
            :class="[
              { active: isActive('/dashboard/student/academic-plan') },
              expanded ? '' : 'justify-center',
            ]"
            class="flex items-center"
