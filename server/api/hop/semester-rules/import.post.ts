...
export default defineEventHandler(async (event) => {
  ...
  for (const rule of parsedRules) {
    try {
      ...
      if (rule.entry_semester === 1) {
        const seededPlans = await seedSemesterOneRulePlans(
          ruleId,
          programId,
          rule.intake_type,
        );
        insertedPlans += seededPlans.length;
      } else {
  ...
  for (const intakeType of uniqueIntakeTypes) {
    const { created } = await ensureBaseRuleForIntake(programId, intakeType);
    if (created) {
      baseRulesCreated++;
      insertedRules++;
    }
