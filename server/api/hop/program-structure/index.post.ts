...
export default defineEventHandler(async (event) => {
  ...
  const { session_id, course_id, semester, course_type, course_group, prerequisite_course_id } = body;
  ...
  const [result] = await pool.query(
    `INSERT INTO program_courses (session_id, course_id, semester, course_type, course_group, prerequisite_course_id)
     VALUES (?, ?, ?, ?, ?, ?)`,
    [session_id, course_id, semester, course_type || 'Core Computing', course_group || null, prerequisite_course_id || null],
  );
